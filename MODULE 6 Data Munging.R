...
#
# (c) Copyright 2015-2025 - AMULET Analytics
# ---------------------------------------------------------------
...
bin_iris$issetosa <- newVars[,1]       # Include setosa binaries
bin_iris$isversicolor <- newVars[,2]   # Include versicolor binaries
bin_iris$isvirginica <- newVars[,3]    # Include virginica binaries

bin_iris$Species <- NULL 

head(bin_iris)

names(bin_iris)
#[1] "Sepal.Length" "Sepal.Width"  "Petal.Length" "Petal.Width" 
#[5] "issetosa"     "isversicolor" "isvirginica" 

# ---------------------------------------------------------------
# Creating binary categorical variables - model.matrix method

bin_iris <- iris       # Make copy of data set first

# Create binary categorical variables for Species. The
# class of the new variables is integer with 1 for TRUE, 
# 0 for FALSE. 

# The formula Species-1 tells model.matrix() to create a design 
# matrix with one column for each level of the Species factor, 
# without subtracting one for the intercept. Use this formula when 
# you want a full one-hot encoding of the factor levels.

species_mat = model.matrix(~ Species - 1, data=iris)

colnames(species_mat) = sub("Species", "", colnames(species_mat))

bin_iris = cbind(bin_iris, species_mat)
bin_iris$Species = NULL

head(bin_iris)

# ---------------------------------------------------------------
# Creating binary categorical variables - fastDummies method
...
# 0 for FALSE. 
#
# By setting remove_selected_columns to TRUE, the 
# Species variable is deleted.

bin_iris <- iris       # Make copy of data set first

bin_iris = dummy_cols(bin_iris, select_columns="Species",
...
y = order(x) # Returns integer vector with element numbers from x
print(y)
#[1] 1 3 4 2
class(y)
#[1] "integer"

print(x[y])    # Display sorted element using subsetting
...
# Example: we take the height variable (measured in cm) and 
# calculate a new height in meters, creating new variable 
# height_m
...
# NOTE: when using the linear regression algorithm with a
# numeric response, only scale the feature variables, 
# leaving the response variable in its original scale. 
