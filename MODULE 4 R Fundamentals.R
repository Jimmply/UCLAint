...
# Also: unsplit() to reverse the effect of split. 
# ---------------------------------------------------------------
...
x <- Sys.time()   # POSIXct format
x
#[1] "2025-01-15 18:56:24 PST"
...
p <- as.POSIXlt(x)   # Cast to POSIXlt
...
# Casting functions can only recognize certain formats
as.POSIXct("January 10, 2012 10:40")   # Error!
...
# set.seed() should precede all functions that use R's random
# number generator
set.seed(1)        

sample(x=1:10, size=4) # Choose 4 items from integers 1:10
#[1] 3 4 5 7

sample(1:10, 4)    # Repeat with no set.seed()
#[1] 3 9 8 5
...
sample(x=letters, size=5) # Choose random letters
#[1] "q" "b" "e" "x" "p"
